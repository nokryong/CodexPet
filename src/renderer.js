...
function updateSpriteMetrics(width, height) {
  ...
  petElement.style.backgroundSize = `${currentWidth * SPRITE_COLS}px ${currentHeight * SPRITE_ROWS}px`;
  applySpriteFrame();
}
...
function applySpriteFrame() {
  ...
  const offsetX = -(column * safeWidth);
  const offsetY = -(state.row * safeHeight);

  petElement.style.backgroundPosition = `${offsetX}px ${offsetY}px`;
}
...
function applySpriteSheet(config, keepState = false) {
  ...
  image.onload = () => {
    ...
    petElement.style.backgroundImage = `url("${config.spriteUrl}")`;
...
function handleClickCandidate() {
  ...
  if (now - lastClickAt <= POINTER_CONFIG.doubleClickMs) {
    ...
    // 더블클릭 시 펫이 점프하면서 Codex 사용량 말풍선을 띄웁니다.
