...
class CodexAccountSwitcher {
  ...
  normalizeUsageWindow(window, fallbackMinutes) {
    ...
    const resetsAt = Number.isFinite(resetAfterSeconds)
      ? Math.floor(Date.now() / 1000 + resetAfterSeconds)
      : Number(window.resets_at ?? window.reset_at);
    const windowMinutes = Number(window.window_minutes ?? window.windowMinutes ?? fallbackMinutes);

    return {
      used_percent: Number.isFinite(usedPercent) ? usedPercent : 0,
      window_minutes: Number.isFinite(windowMinutes) ? windowMinutes : fallbackMinutes,
  ...
  async fetchCurrentUsage() {
    ...
    return {
      ...
      rateLimits: {
        plan_type: planType,
        primary: this.normalizeUsageWindow(rateLimit.primary_window || rateLimit.primaryWindow, 300),
        secondary: this.normalizeUsageWindow(rateLimit.secondary_window || rateLimit.secondaryWindow, 10080),
      },
    };
  }
}
