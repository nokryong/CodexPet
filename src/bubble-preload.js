...
contextBridge.exposeInMainWorld("bubbleApi", {
  ...
  sendAction: (actionId, payload = null) => {
    ipcRenderer.send(BUBBLE_CHANNELS.ACTION, actionId, payload);
  },
  onAppearance: (handler) => {
    if (typeof handler !== "function") return () => {};
    const listener = (_event, data) => handler(data);
    ipcRenderer.on(BUBBLE_CHANNELS.APPEARANCE, listener);
    return () => ipcRenderer.removeListener(BUBBLE_CHANNELS.APPEARANCE, listener);
  },
});
